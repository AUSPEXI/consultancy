...
export function BlogHero({ title, category, className = '', compact = false }: BlogHeroProps) {
  ...
  return (
    <div className={`relative w-full min-h-full bg-[#0B0E14] overflow-hidden flex items-center justify-center ${compact ? 'p-4' : 'p-8'} ${className}`}>
      ...
      <div className={`relative z-10 w-full min-h-full flex ${compact ? 'flex-col md:flex-row items-start md:items-center p-4 gap-4' : 'flex-col sm:flex-row items-center p-6 sm:p-8 gap-6 sm:gap-8'} rounded-xl border border-pink-500/20 bg-zinc-950/20 shadow-[0_0_30px_-5px_rgba(236,72,153,0.15)]`}>

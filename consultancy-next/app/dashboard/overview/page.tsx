...
export default function OverviewPage() {
  ...
  // Free tier sees the Overview READ-ONLY: the full UI renders (data-fetch
  // effects above stay Starter-gated, so a Free user simply sees empty states),
  // but every paid action is disabled with an upgrade nudge. Admins + Starter+
  // get full interactivity.
    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-20 relative">
      ...
      {isReadOnly && (
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 rounded-xl border border-pink-500/30 bg-pink-500/5 px-4 py-3">
          ...
          <a
            href="/#pricing"
            className="shrink-0 rounded-lg bg-pink-600 hover:bg-pink-500 px-4 py-2 text-sm font-semibold text-white transition-colors"
          >
            Upgrade to Starter
          </a>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="lg:col-span-2 bg-zinc-900/50 border border-zinc-800 rounded-xl p-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <button onClick={generateShadowLink} disabled={isGeneratingLink || !shadowUrl.trim() || isReadOnly} title={isReadOnly ? 'Upgrade to Starter to generate Shadow Links' : undefined} className="bg-pink-600 hover:bg-pink-700 disabled:opacity-50 disabled:cursor-not-allowed text-white px-6 py-3 rounded-lg text-sm font-medium transition-colors whitespace-nowrap flex items-center justify-center gap-2">
            ...
            <div className="mt-4 space-y-4">
              <div className="p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-lg flex items-center justify-between">
                ...
                <div className="flex gap-2 ml-4">
                  ...
                  <button onClick={handleSyncCMS} disabled={isSyncingCMS || isReadOnly} title={isReadOnly ? 'Upgrade to Starter to sync to your CMS' : undefined} className="px-3 py-1.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 rounded-md text-xs font-medium transition-colors whitespace-nowrap flex items-center gap-1.5 disabled:opacity-50 disabled:cursor-not-allowed">

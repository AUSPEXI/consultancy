      ...
      for (const collName of collectionsToClear) {
        try {
          ...
          while (processed < snapshot.size) {
            ...
            snapshot.docs.slice(processed, processed + batchSize).forEach(d => batch.delete(d.ref));
            await batch.commit();
            processed += batchSize;
          }
        } catch (err) { console.error(`Failed to clear collection ${collName}:`, err); }
      }
    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="bg-zinc-900/50 border border-zinc-800 rounded-3xl p-8 md:p-12 relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <p className="text-zinc-400 text-lg mb-8 leading-relaxed">Use these tools to manually manipulate your account state for testing purposes. These actions are irreversible.</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <button onClick={seedHistoricalData} disabled={isSeeding || isResetting} className="group p-6 bg-zinc-900 border border-zinc-800 rounded-2xl text-left hover:border-pink-500/50 transition-all disabled:opacity-50">
              ...
              <p className="text-xs text-zinc-500 leading-relaxed">Injects 15 days of historical audit data to populate the Z-Score Drift charts and UMAP clusters.</p>
            </button>

            <button onClick={hardReset} disabled={isResetting || isSeeding} className="group p-6 bg-zinc-900 border border-zinc-800 rounded-2xl text-left hover:border-rose-500/50 transition-all disabled:opacity-50">
              ...
              <p className="text-xs text-zinc-500 leading-relaxed">Deletes all audits, scans, and facts. Resets onboarding status and clears local chat memory.</p>
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
        ...
        <div className="md:col-span-4 bg-zinc-900/50 border border-zinc-800 rounded-3xl p-8">
          ...
          <div className="space-y-4">
            {[['Captured Leads', '1,240'], ['Sequence Replies', '12%'], ['Conv. Rate', '8.4%']].map(([label, value]) => (
              <div key={label} className="flex justify-between items-center p-3 bg-zinc-950 border border-zinc-800 rounded-xl">
                <span className="text-xs text-zinc-500">{label}</span>
                <span className={`text-sm font-bold ${label === 'Conv. Rate' ? 'text-emerald-500' : 'text-white'}`}>{value}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-zinc-900/50 border border-zinc-800 rounded-3xl p-8">
        ...
        <p className="text-zinc-400 text-sm mb-6">We&apos;ve automated the generation of &ldquo;Hot Pink Latent Space&rdquo; social preview images. Visit any post&apos;s preview URL below and take a screenshot, or use an automated service to fetch these URLs.</p>

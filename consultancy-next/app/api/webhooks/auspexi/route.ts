...
/**
 * Inbound webhook — receives content from the auspexi.com internal server.
 * Authenticate with header: x-auspexi-secret: <AUSPEXI_WEBHOOK_SECRET env var>
 *
 * Payload shape:
 * {
...
export async function POST(req: NextRequest) {
  // Auth check — optional if AUSPEXI_WEBHOOK_SECRET is set
  const secret = req.headers.get('x-auspexi-secret') || '';
  const expectedSecret = process.env.AUSPEXI_WEBHOOK_SECRET;
  ...
  try {
    ...
    console.error('[webhook/auspexi]', err);

...
export const DASHBOARD_TOOLS: DashboardTool[] = [
  ...
  },
  {
    ...
    status: 'active',
    purpose: 'Multi-Agent Orchestration Guild. Full pipeline: Crawler (Auspexi Neural Crawler) → Extraction Agent (fact isolation, no hallucinations) → Schema Agent (JSON-LD generator) → Synthesis Agent (GEO-optimised article). Output published directly to CMS via webhook.',

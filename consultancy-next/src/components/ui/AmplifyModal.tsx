...
export const AmplifyModal: React.FC<AmplifyModalProps> = ({ fact, onClose }) => {
  ...
  const connectedAccounts = (userData as any)?.connectedSocials || [];
  ...
  const handlePublish = async (platform: string, text: string) => {
    ...
    if (userData?.cmsWebhookUrl) {
      try {
        const response = await fetch(userData.cmsWebhookUrl, {
          ...
          body: JSON.stringify({ event: 'SOCIAL_PUBLISH', timestamp: new Date().toISOString(), payload: { platform, content: text } })
        });
        ...
        setPublishStatus(prev => { const s = { ...prev }; delete s[platform]; return s; });
        return;
      }
    } else {
    ...
    setTimeout(() => {
      ...
      if (auth.currentUser) logAuditAction(auth.currentUser.uid, 'CONTENT_AUTO_PUBLISHED', { platform });
    }, 500);
  };
    ...
    <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
      <div className="bg-zinc-950 border border-zinc-800 rounded-xl w-full max-w-6xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
        <div className="p-4 border-b border-zinc-800 flex items-center justify-between bg-zinc-900/50">
          <div>
            <h2 className="text-lg font-semibold text-white">Omnichannel Amplifier</h2>
            <p className="text-sm text-zinc-400 mt-1 truncate max-w-xl">Seeding: &ldquo;{fact}&rdquo;</p>
          </div>
          <button onClick={onClose} className="text-zinc-400 hover:text-white transition-colors p-2 hover:bg-zinc-800 rounded-md">
            <X className="w-5 h-5" />
          </button>
        </div>
        <div className="flex-1 overflow-y-auto p-6">
          {isGenerating ? (
            ...
            <div className="flex flex-col items-center justify-center h-64 space-y-4 text-rose-400">
              <p>{error}</p>
              <button onClick={onClose} className="px-4 py-2 bg-zinc-800 hover:bg-zinc-700 text-white rounded-md transition-colors">Close</button>
            </div>
          ) : content ? (
              ...
              {platforms.map(({ key, label, icon: Icon, color }) => (
                <div key={key} className="bg-zinc-900 border border-zinc-800 rounded-lg overflow-hidden flex flex-col">
                  <div className="px-4 py-3 border-b border-zinc-800 bg-zinc-900/80 flex items-center justify-between">
                    ...
                    <button onClick={() => handleCopy(key, content[key])} className="text-xs flex items-center gap-1.5 text-zinc-400 hover:text-white transition-colors bg-zinc-800/50 hover:bg-zinc-700 px-2 py-1 rounded">
                      {copiedPlatform === key ? (
                  ...
                  <div className="p-4 flex-1">
                    <textarea readOnly value={content[key]} className="w-full h-48 bg-transparent text-sm text-zinc-300 resize-none focus:outline-none" />
                  </div>
                  <div className="px-4 py-3 border-t border-zinc-800 bg-zinc-900/50 flex justify-end">
                    {connectedAccounts.includes(key) ? (
                      <button onClick={() => handlePublish(key, content[key])} disabled={publishStatus[key] === 'publishing' || publishStatus[key] === 'published'} className="px-3 py-1.5 bg-zinc-800 hover:bg-zinc-700 text-white text-xs font-medium rounded-md transition-colors flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
                      ...
                      <button onClick={() => handleConnect(key)} className="px-3 py-1.5 border border-zinc-700 hover:bg-zinc-800 text-zinc-300 text-xs font-medium rounded-md transition-colors">
                        Connect Account
                      </button>
                    )}
                  </div>

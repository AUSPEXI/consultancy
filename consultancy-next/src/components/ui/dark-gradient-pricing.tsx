...
const Benefit = ({ text, checked }: BenefitProps) => {
  return (
    <div className="flex items-start gap-4">
      {checked ? (
        <span className="mt-0.5 flex shrink-0 items-center justify-center rounded-full bg-pink-500/20 text-pink-400 p-1">
...
export const PricingCard = ({
  ...
  return (
    ...
    >
      ...
      >
        {isPopular && (
          <div className="absolute -top-4 left-0 right-0 mx-auto w-fit rounded-full bg-pink-500 px-4 py-1 text-xs font-bold uppercase tracking-widest text-white shadow-lg shadow-pink-500/30">
            Most Popular
          </div>
        )}
        ...
        <div className="mb-8">
          <h3 className={cn(
            ...
            isPopular ? "text-pink-400" : "text-zinc-400"
          )}>
            {tier}
          </h3>
          <div className="mb-4 flex flex-wrap items-baseline gap-1 md:gap-2">
            <span className="text-4xl lg:text-5xl font-bold font-heading text-white tracking-tight break-all">
              {priceAmount}
            </span>
            {pricePeriod && (
              <span className="text-base md:text-lg font-medium text-zinc-500 shrink-0">
                /{pricePeriod}
              </span>
            )}
          </div>
